...
<c:if test = "${count>0}">
    ...
    <c:if test="${startPage > 10}">
        <a href="/pageWep/board/list.do?pageNum=${startPage - 10 }">[이전]</a>
    </c:if>

    <c:forEach var="i" begin="${startPage}" end="${endPage}">
       <a href="/pageWep/board/list.do?pageNum=${i}">[${i}]</a>
    </c:forEach>

    <c:if test="${endPage < pageCount}">
        <a href="/pageWep/board/list.do?pageNum=${startPage + 10}">[다음]</a>
    </c:if>
</c:if>
