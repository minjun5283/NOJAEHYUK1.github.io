...
<c:if test = "${count>0}">
    ...
    <c:if test="${startPage > 10}">
        <a href="/pageWep/cboard/searchlist.do?pageNum=${startPage - 10 }">[이전]</a>
    </c:if>

    <c:forEach var="i" begin="${startPage}" end="${endPage}">
       <a href="/pageWep/cboard/searchlist.do?pageNum=${i}&search=${search}&ch=${ch}">[${i}]</a>
    </c:forEach>

    <c:if test="${endPage < pageCount}">
        <a href="/pageWep/cboard/searchlist.do?pageNum=${startPage + 10}">[다음]</a>
    </c:if>
</c:if>
	...
	<form action = "/pageWep/cboard/searchlist.do" method = "post">
		<select name = "ch">
			...
			<option value = "cboard_subject">제목 </option>
		</select>
