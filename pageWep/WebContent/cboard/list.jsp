...
<tr>
    ...
    <c:if test="${memId != null}">
    		<a href="/pageWep/cboard/writeForm.do"> 글쓰기 </a>
    		<a href="/pageWep/cboard/myWrite.do">내 글</a>
   	</c:if>
    </td>
</table>
...
<c:if test = "${count > 0}" >
	<table border="1" width="760" height="50" cellpadding="0" cellspacing="0" align="center"> 
		<tr height="50"> 
			<td align="center"  width="50"  >번 호</td> 
			<td align="center"  width="160" >코디 </td>
	...
	<tr height="50">
    	...
    	<td width="160" height="50"> 
    	    ...
    	    <img src ="${outImg[i-1]}" border="1" width="50" height="50">
    		<img src ="${topImg[i-1]}" border="1" width="50" height="50">
    		<img src ="${bottomImg[i-1]}" border="1" width="50" height="50">
    	</td>
    	<td  width="250" height="50" >
			<c:if test ="${article.cboard_relevel > 0 }">
				<img src="images/level.gif" width="${5*article.cboard_relevel}" height="16">
			...
			<a href="/pageWep/cboard/content.do?num=${article.cboard_num}&pageNum=${currentPage}">
				${article.cboard_subject}</a> 
				<c:if test = "${article.cboard_readcount >= 20}">
					<img src="images/hot.gif" border="0"  height="16"> </td>
				</c:if>
		</td>
		<td align="center"  width="100">${article.member_id}</td>
    	<td align="center"  width="150">${article.cboard_reg_date}</td>
    	<td align="center"  width="50">${article.cboard_readcount}</td>
    </tr>
	</c:forEach>
</table>
</c:if>
...
<c:if test = "${count>0}">
    ...
    <c:if test="${startPage > 10}">
        <a href="/pageWep/cboard/list.do?pageNum=${startPage - 10 }">[이전]</a>
    </c:if>

    <c:forEach var="i" begin="${startPage}" end="${endPage}">
       <a href="/pageWep/cboard/list.do?pageNum=${i}">[${i}]</a>
    </c:forEach>

    <c:if test="${endPage < pageCount}">
        <a href="/pageWep/cboard/list.do?pageNum=${startPage + 10}">[다음]</a>
    </c:if>
</c:if>

	<form action = "/pageWep/cboard/searchlist.do" method = "post">
		<select name = "ch">
...
<!-- Footer -->
<footer>
	<p class="credits">&copy;글로벌아이티 인재개발 의류몰 <a href="../sitemap.do" title="sitemap">사이트맵</a>
	</p>
</footer>
<!-- End Footer -->
